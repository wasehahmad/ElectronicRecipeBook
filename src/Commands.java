...
public class Commands{
  ...
  public boolean planMeal(String time,String requiredItems,String excludedItems,String cuisine){
    ...
    try{
        ...
        for(int j=0;j<typeCon.getList("Salad").size() && !found;j++){
          for(int k=0;k<typeCon.getList("Appetizer").size() && !found;k++){
            for(int m=0;m<typeCon.getList("Appetizer").size() && !found;m++){
              ...
              if(meal.getTotal()>Integer.parseInt(time)){
              }
              else{
      ...
      if(tree.size() !=0 && !found){
        LinkedList<Meal> optimalMeals= tree.search(Integer.parseInt(time));
        optimalMeal =optimalMeals.get(0);
        found=true;
      }
      if(optimalMeal!=null){
      ...
      for(int p=0;p<4;p++){
      display(tree.search(Integer.parseInt(time)).get(0).getContentsMeal().get(p).getName());
       System.out.println();
